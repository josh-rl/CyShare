...
public class passenger_create_request extends AppCompatActivity implements OnMapReadyCallback {
    /**
    ...
    private AutocompleteSupportFragment passDestination;
    private AutocompleteSupportFragment passLocation;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_passenger_create_request);
        ...
        // init fields
        departureTime = findViewById(R.id.etDepTimePRB);
        departureDate = findViewById(R.id.etDepDatePRB);
        passengerCount = findViewById(R.id.etPassCountPRB);
        toHome = findViewById(R.id.button_to_lobbyPRB);
        ...
        passDestination = (AutocompleteSupportFragment) getSupportFragmentManager().findFragmentById(R.id.autocomplete_destinationPRB);
        passLocation = (AutocompleteSupportFragment) getSupportFragmentManager().findFragmentById(R.id.autocomplete_locationPRB);

        // Initialize Google MapFragment
        mapFragment = (SupportMapFragment) getSupportFragmentManager().findFragmentById(R.id.mapPRB);
        mapFragment.getMapAsync(this);
        destMarker = null;
        currMarker = null;

        // Specify the types of place data to return.
        passLocation.setPlaceFields(Arrays.asList(Place.Field.ID, Place.Field.LAT_LNG, Place.Field.NAME));
        passDestination.setPlaceFields(Arrays.asList(Place.Field.ID, Place.Field.LAT_LNG, Place.Field.NAME));
        //endregion

        // Set up a PlaceSelectionListener to handle the response
        passLocation.setOnPlaceSelectedListener(new PlaceSelectionListener() {
            @Override
...
//                Log.i("PlacesApi", "Place: " + currLocation.latitude + "\n" + currLocation.longitude);
        ...
        passDestination.setOnPlaceSelectedListener(new PlaceSelectionListener() {
            @Override
            public void onPlaceSelected(@NonNull Place place) {
...
//
    ...
    private boolean validate() throws JSONException {
        if (destLocation != null && currLocation != null && departureTimeStr != "" && departureDateStr != "" && passCountStr != "") {
            ...
            backend_io.serverPostObject(volleyQueue, "http://coms-309-031.cs.iastate.edu:8080/PassengerBooking/add", toServer);
